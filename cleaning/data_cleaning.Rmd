...
train_side <- combined %>% dplyr::filter(!is.na(Side))
missing_side <- combined %>% dplyr::filter(is.na(Side))
...
predicted_side <- predict(side_model, missing_side, type = "class")
predicted_side <- factor(predicted_side, levels = levels(combined$Side))
combined$Side[is.na(combined$Side)] <- predicted_side
